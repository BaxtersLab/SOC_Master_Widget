...
no other test covers — a typo in it would only ever surface as a launch-time
`symbol lookup error: undefined symbol: __libc_pthread_init`, which reads like
a Python bug and is not one.

It matters more here than in any sibling app. The board's whole job is spawning
HRT, GGUF Chatbox and SOC Ultralight as children, and the environment it holds
is the environment they inherit — so a hole in this block is a hole in all four
apps at once, and the failure would surface in a child rather than here.
